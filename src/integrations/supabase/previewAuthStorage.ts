...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // `timer` is referenced inside `finish` below before it's declared —
      // fine, since `finish` is only ever called asynchronously (by the
      // timeout itself or a later message), strictly after the `const timer`
      // assignment at the bottom of this block has run.
